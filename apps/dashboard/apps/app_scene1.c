...
static bool touchDownFlag = false;
...
{
    if (appData.scene_state == APP_SCENE_STATE_SHOW_HELP ||
        appData.scene_state == APP_SCENE_STATE_SWITCH_SCENE_1_2_0)
        return;
    ...
    switch (appData.state)
    {
        ...
        {
            if ((lastX > evt->x) && 
                (lastX - evt->x > SWIPEX_THRESHOLD_PIXELS) &&
                (evt->y < TOP_EDGE_TOUCH_Y_MAX || evt->y > BOTTOM_EDGE_TOUCH_Y_MAX) &&
                (lastY < TOP_EDGE_TOUCH_Y_MAX || lastY > BOTTOM_EDGE_TOUCH_Y_MAX))
            {
                    appData.scene_state = APP_SCENE_STATE_SWITCH_SCENE_1_2_0;
            }
            else if ((lastY < evt->y) && 
                (evt->y - lastY > SWIPEY_THRESHOLD_PIXELS) &&
                (evt->x > LEFT_EDGE_TOUCH_X_MAX && evt->x < RIGHT_EDGE_TOUCH_X_MIN) &&
                (lastX > LEFT_EDGE_TOUCH_X_MAX && lastX < RIGHT_EDGE_TOUCH_X_MIN))
            {
        ...
        {
            if ((lastX < evt->x) && 
                (evt->x - lastX > SWIPEX_THRESHOLD_PIXELS) &&
                (evt->y < TOP_EDGE_TOUCH_Y_MAX || evt->y > BOTTOM_EDGE_TOUCH_Y_MAX) &&
                (lastY < TOP_EDGE_TOUCH_Y_MAX || lastY > BOTTOM_EDGE_TOUCH_Y_MAX))
            {
                    appData.scene_state = APP_SCENE_STATE_SWITCH_SCENE_1_2_0;
